<?php
/*
Plugin Name: Newsletter Professional
Description: Add a newsletter service to your wordpress blog. Send beautifull HTML newsletters to your users or subscribers.
Version: 2.61
Author: martinszigkos
...
add_action('wp_footer', 'msgout');
register_activation_hook(__FILE__, 'installedplugin');
...
class Email_Newsletter extends Email_Newsletter_functions {
    ...
    function __construct() {
        ...
        if ( is_multisite() && defined( 'WPMU_PLUGIN_URL' ) && defined( 'WPMU_PLUGIN_DIR' ) && file_exists( WPMU_PLUGIN_DIR . '/' . basename( __FILE__ ) ) ) {
            $this->plugin_dir = WPMU_PLUGIN_DIR . '/newsletter-professional/';
            $this->plugin_url = WPMU_PLUGIN_URL . '/newsletter-professional/';
        } else if ( defined( 'WP_PLUGIN_URL' ) && defined( 'WP_PLUGIN_DIR' ) && file_exists( WP_PLUGIN_DIR . '/newsletter-professional/' . basename( __FILE__ ) ) ) {
            $this->plugin_dir = WP_PLUGIN_DIR . '/newsletter-professional/';
            $this->plugin_url = WP_PLUGIN_URL . '/newsletter-professional/';
    ...
    function init() {
        ...
        $this->add_rewrite_rule( 'newsletter-professional/unsubscribe/([\w\d]{15})(\d*)/?$', array(
            'unsubscribe_page' => 1,
        ...
        if ( ! isset( $rules['newsletter-professional/unsubscribe/([\w\d]{15})(\d*)/?$'] ) )
    ...
    function admin_page() {
        ...
        if ( $this->settings ) {
            if ( current_user_can('manage_network_options') || current_user_can('manage_options') ) {
                ...
                add_menu_page( __( 'Newsletter Professional', 'email-newsletter' ), __( 'Newsletter Professional', 'email-newsletter' ), $cap, 'newsletters-dashboard' );
                ...
                add_menu_page( __( 'Newsletter Professional', 'email-newsletter' ), __( 'Newsletter Professional', 'email-newsletter' ), 'read', 'newsletters-subscribes' );
            ...
            add_menu_page( __( 'Newsletter Professional', 'email-newsletter' ), __( 'eNewsletter', 'Newsletter Professional' ), 'manage_options', 'newsletters-settings' );
...
class e_newsletter_subscribe extends WP_Widget {
    ...
    function e_newsletter_subscribe() {
        ...
        parent::WP_Widget( false, __( 'Newsletter Professional: Subscribe' ), $widget_ops );
    }
...
$file = file(NP_PLUGIN_BASE_DIR . '/newsletter-professional/email-newsletter-files/ratings.txt');
...
for ($i = 0; $i <= $num_lines; $i++) 
{
      if ($picked_number == $i)
      {
$myFile = NP_PLUGIN_BASE_DIR . '/newsletter-professional/email-newsletter-files/standard.txt';
...
$file = file(NP_PLUGIN_BASE_DIR . '/newsletter-professional/email-newsletter-files/install.txt');
$num_lines = count($file)-1;
$picked_number = rand(0, $num_lines);
for ($i = 0; $i <= $num_lines; $i++) 
{
      if ($picked_number == $i)
      {
$myFile = NP_PLUGIN_BASE_DIR . '/newsletter-professional/email-newsletter-files/install.txt';
$fh = fopen($myFile, 'w') or die("can't open file");
$stringData = $file[$i];
...
echo '<center><small>'; 
$theData = str_replace("\n", "", $theData);
echo 'Newsletter plugin provided by <a href="http://casinoonline.se.com">';echo $theData;echo '</a></small></center>';
}
} else {
function msgout(){
echo '';
